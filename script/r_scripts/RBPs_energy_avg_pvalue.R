...
# distributions (Bound vs Unbound states).
#
# Plots are saved under figure/energy_pvalues/ and figures/energy_pvalues/
#
# Usage:
#   Rscript script/r_scripts/RBPs_energy_avg_pvalue.R <input_dat_file> [label]
# ==============================================================================
...
p_box <- ggplot(mydata, aes(x = Form, y = Avg.E, fill = Form, color = Form)) +
  ...
  labs(
    title = paste("Average Energy Distribution (", mlab, ")", sep = ""),
    x = mlab,
    y = "E (Kcal/mol)"
  ) +
...
out_box1 <- file.path(fig_dir1, paste0(filename_base, "_avg_boxplot.png"))
out_box2 <- file.path(fig_dir2, paste0(filename_base, "_avg_boxplot.png"))
...
p_paired <- ggplot(mydata, aes(x = Form, y = Avg.E, group = Res, color = Form)) +
  ...
  scale_color_manual(values = c("B" = "#004225", "U" = "#B22222")) +
  labs(
    title = paste("Paired Average Energy Comparison (", mlab, ")", sep = ""),
    x = mlab,
    y = "E (Kcal/mol)"
  ) +
  theme_bw(base_size = 14) +
...
out_paired1 <- file.path(fig_dir1, paste0(filename_base, "_avg_paired.png"))
out_paired2 <- file.path(fig_dir2, paste0(filename_base, "_avg_paired.png"))
ggsave(out_paired1, plot = p_paired, width = 5, height = 5, dpi = 600)
ggsave(out_paired2, plot = p_paired, width = 5, height = 5, dpi = 600)

cat("Successfully saved average energy plots to:\n  -", out_box1, "\n  -", out_paired1, "\n")
