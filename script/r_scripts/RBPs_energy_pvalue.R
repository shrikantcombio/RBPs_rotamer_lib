...
# side-chain energy distributions (Bound vs Unbound states) from JSON summary files.
#
# Usage:
#   Rscript script/r_scripts/RBPs_energy_pvalue.R [json_input_file] [region: INT|NINT|AA_INT|AA_NINT|AROMATIC_INT|AROMATIC_NINT]
# ==============================================================================
...
if (grepl("amino_acid", json_file, ignore.case = TRUE) || grepl("AA", region_mode, ignore.case = TRUE)) {
  ...
  if (grepl("NINT", region_mode, ignore.case = TRUE)) {
    ...
    res_ids <- df_raw$AA
    mlab <- paste(mlab, "Non-interface (Total E)")
  } else {
    u_vals <- df_raw$IU_DELG
    b_vals <- df_raw$IB_DELG
    res_ids <- df_raw$AA
    mlab <- paste(mlab, "Interface (Total E)")
  }
} else {
  # PDB Level
  ...
  if (grepl("NINT", region_mode, ignore.case = TRUE)) {
    u_vals <- df_raw$NU_DELG
    b_vals <- df_raw$NB_DELG
    mlab <- "PDB Non-interface (Total E)"
  } else {
    u_vals <- df_raw$IU_DELG
    b_vals <- df_raw$IB_DELG
    mlab <- "PDB Interface (Total E)"
  }
}
...
filename_base <- paste0("total_energy_", tolower(gsub("[^A-Za-z0-9]", "_", region_mode)))
...
p_box <- ggplot(mydata, aes(x = Form, y = Avg.E, fill = Form, color = Form)) +
  ...
  labs(title = paste("Total Energy Distribution (", mlab, ")", sep = ""), x = "State", y = "E (kcal/mol)") +
...
p_paired <- ggplot(mydata, aes(x = Form, y = Avg.E, group = Res, color = Form)) +
  ...
  scale_color_manual(values = c("B" = "#004225", "U" = "#B22222")) +
  labs(title = paste("Paired Total Energy Comparison (", mlab, ")", sep = ""), x = "State", y = "E (kcal/mol)") +
  theme_bw(base_size = 14) +
  theme(plot.title = element_text(size = 13, color = "red", face = "bold.italic", hjust = 0.5), axis.title = element_text(size = 14, face = "bold"), legend.position = "none")
...
cat("Successfully computed t-tests and saved plots for region:", mlab, "\n")
